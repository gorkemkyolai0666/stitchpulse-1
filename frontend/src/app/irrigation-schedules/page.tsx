...
import {
  formatDateTime,
  formatIrrigationScheduleStatus,
  formatIrrigationScheduleCategory,
} from '@/lib/utils';
...
export default function IrrigationSchedulePage() {
  const { token } = useAuth();
  const [items, setItems] = useState<IrrigationScheduleItem[]>([]);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await api.irrigationSchedules.create(token, {
        title: form.title,
    ...
    <AppLayout>
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h1 className="font-display text-3xl text-primary">Sulama Kontrol Kontrol Listeleri</h1>
        ...
        {showForm && (
          <Card className="atelier-card">
            <CardHeader>
              <CardTitle className="font-display">Sulama Kontrol Planı Ekle</CardTitle>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleCreate} className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <select id="category" value={form.category} onChange={(e) => update('category', e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
                    {CATEGORIES.map((c) => (
                      <option key={c} value={c}>{formatIrrigationScheduleCategory(c)}</option>
                    ))}
                  </select>
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <select id="status" value={form.status} onChange={(e) => update('status', e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
                    {STATUSES.map((s) => (
                      <option key={s} value={s}>{formatIrrigationScheduleStatus(s)}</option>
                    ))}
                  </select>
                </div>
        ...
        {!loading && !error && items.length === 0 && (
          <EmptyState title="Sulama Kontrol planı yok" description="Henüz sıfırlama kontrol listesi eklenmemiş." />
        )}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {formatIrrigationScheduleCategory(item.category)} · {item.zone || 'Genel'} · {formatDateTime(item.scheduledAt)}
                      </p>
                    </div>
                  </div>
                  <Badge variant="secondary">{formatIrrigationScheduleStatus(item.status)}</Badge>
                </CardContent>
              </Card>
            ))}
          </div>
