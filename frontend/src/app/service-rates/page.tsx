...
export default function ServiceRatesPage() {
  const { token } = useAuth();
  const [tiers, setTiers] = useState<ServiceRate[]>([]);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await api.serviceRates.create(token, {
        title: form.title,
            ...
            <CardContent>
              <form onSubmit={handleCreate} className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2 sm:col-span-2">
                  ...
                  >
                    {STATUSES.map((s) => (
                      <option key={s} value={s}>{formatServiceRateStatus(s)}</option>
                    ))}
                  </select>
                </div>
                  ...
                  <tbody>
                    {tiers.map((tier) => (
                      <tr key={tier.id} className="border-b border-muted hover:bg-muted/30">
                        ...
                        <td className="p-3">{formatServiceCategory(tier.rateCategory)}</td>
                        ...
                        <td className="p-3">
                          <Badge variant="secondary">{formatServiceRateStatus(tier.status)}</Badge>
                        </td>
                      </tr>
                    ))}
                  </tbody>
