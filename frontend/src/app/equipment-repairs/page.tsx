...
export default function EquipmentRepairPage() {
  const { token } = useAuth();
  const [items, setItems] = useState<EquipmentRepairItem[]>([]);
  ...
  const load = () => {
    ...
    setLoading(true);
    setError(false);
    Promise.all([api.equipmentRepairs.list(token), api.greenhouseBays.list(token)])
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await api.equipmentRepairs.create(token, {
              ...
              <form onSubmit={handleCreate} className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <select id="priority" value={form.priority} onChange={(e) => update('priority', e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
                    {PRIORITIES.map((p) => (
                      <option key={p} value={p}>{formatEquipmentRepairPriority(p)}</option>
                    ))}
                  </select>
                </div>
                <div className="space-y-2">
                  ...
                  <select id="status" value={form.status} onChange={(e) => update('status', e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
                    {STATUSES.map((s) => (
                      <option key={s} value={s}>{formatEquipmentRepairStatus(s)}</option>
                    ))}
                  </select>
                </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {item.greenhouseBay?.name || 'İstasyon belirtilmemiş'} · {item.greenhouseBay?.zone} · {formatEquipmentRepairPriority(item.priority)} · {formatDateTime(item.reportedAt)}
                      </p>
                    </div>
                  </div>
                  <Badge variant="secondary">{formatEquipmentRepairStatus(item.status)}</Badge>
                </CardContent>
              </Card>
            ))}
          </div>
